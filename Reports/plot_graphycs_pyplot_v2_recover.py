...
df['Distance'] = np.sqrt((df['Recovered Easting'] - df['Preplot Easting'])**2 + (df['Recovered Northing'] - df['Preplot Northing'])**2)

# Calculate the percent of nodes that fell between 0 to 4.99m, 5 to 9.99m, 10 to 14.99m and above 15m respectively
mask = pd.notna(df['Recovered Easting']) & df['Recovered Easting'] 
...
ax.set_xticks([])
ax.set_yticks([])
ax.set_title('Recovered to Preplot Distance with Azimuth')

# Add text with percent of nodes that fell between 0 to 4.99m, 5 to 9.99m, 10 to 14.99m and above 15m respectively
ax.text(1.05, 0.5,
        f'{nodes_0_to_4_99m} Nodes < 5m: {percent_0_to_4_99m}%',
        transform=ax.transAxes,
        color='green')
ax.text(1.05, 0.45,
        f'{nodes_5_to_9_99m} Nodes >= 5m < 10m: {percent_5_to_9_99m}%',
        transform=ax.transAxes,
        color='red')
ax.text(1.05, 0.4,
        f'{nodes_10_to_14_99m} Nodes >=10m < 15m: {percent_10_to_14_99m}%',
        transform=ax.transAxes,
        color='purple')
ax.text(1.05, 0.35,
        f'{nodes_above_15m} Nodes >= 15m: {percent_above_15m}%',
        transform=ax.transAxes,
        color='black')
