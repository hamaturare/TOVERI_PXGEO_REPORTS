...
def get_latest_files(file_folder_path):
    try:
        # Get a list of all deployment files
        latest_files_deployed = glob.glob(os.path.join(file_folder_path, 'NodeProduction_Deployed_*.xlsx'))
        # Sort the list of files by creation time (most recent first)
        latest_files_deployed.sort(key=os.path.getctime, reverse=True)

        # Get a list of all recovery files
        latest_files_recovered = glob.glob(os.path.join(file_folder_path, 'NodeProduction_Recovered_*.xlsx'))
        # Sort the list of files by creation time (most recent first)
...
deployed_file, recovered_file = get_latest_files(file_folder_path)
